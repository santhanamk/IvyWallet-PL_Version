...
class WriteAccountsActTest: IvyAndroidTest() {
    ...
    fun testSaveUpdateAccount() = runTest {
        ...
        val accountToSave = account().copy(sync = Sync(SyncState.Syncing, syncTime))
        writeAccountsAct(Modify.save(accountToSave))

        val createdAccountFromDb = accountDao.findAllBlocking().first()
       assertk.assertThat(createdAccountFromDb).transformToAccount().isEqualTo(accountToSave)

        val updatedAccount  = accountToSave.copy(name = "updated", currency = "CAD")
        ...
        assertThat(updatedAccountFromDb).transformToAccount().isEqualTo(updatedAccount)
    ...
    private fun Assert<AccountEntity>.transformToAccount(): Assert<Account> {
        return transform {
            Account(id = UUID.fromString(it.id),
                name = it.name,
                currency = it.currency,
                color = it.color,
                icon = it.icon,
                excluded = it.excluded,
                folderId = it.folderId?.let {
                                            UUID.fromString(it)
                },
